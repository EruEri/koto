...
use crate::{spotify::{self, SpotifySearchType, SpotifySearchResultItem}, config::koto_base_dir, sql::Date};
...
pub struct Artist {
    ...
    pub(crate) last_album_release_date: String,
...
impl Artist {
    ...
    pub async fn update(&mut self, spotify: &spotify::Spotify) -> bool {
        ...
        let latest_realease_date = match Date::from_str(&lastest.release_date) {
            Some(e) => e,
            None => return false,
        };
        let should_update = latest_realease_date > Date::from_str(&self.last_album_release_date).unwrap_or(Date::unix_epoch());
        let () = match should_update {
            true => {
                self.last_album = lastest.name;
                self.last_album_spotify_id = lastest.id;
                self.last_album_release_date = latest_realease_date.to_string();
...
impl Artists {
    pub async fn update(&mut self, id: bool, artist: &Option<String>) {
        ...
        let mut artists = match artist {
            Some(name) => self.artists.iter_mut().find(|elt| match id {
    ...
    pub fn save(&self) {
        ...
        let file = OpenOptions::new().create(false).truncate(true).write(true).read(false).open(path).unwrap();
    ...
    pub fn deserialize() -> Option<Self> {
        ...
        let file = OpenOptions::new().create(false).truncate(false).write(false).read(true).open(path).unwrap();
