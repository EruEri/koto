use std::{collections::HashSet, path::{PathBuf, Path}, fs::{OpenOptions, read_dir}, io::Write, fmt::Display};
...
impl CreateM3U {
    pub fn run(self) -> () {
        let CreateM3U {include_extension, exclude_extension, output, directories} = self;
